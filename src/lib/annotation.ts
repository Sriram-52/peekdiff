// Width is capped at a comfortable reading measure but never exceeds the
// column it renders in (split view puts annotations inside one diff side), so
// on a narrow pane the card shrinks to fit instead of overflowing the viewport.
// `min-w-0` lets the flex content column shrink so long code/tokens wrap or
// scroll internally rather than forcing the card wider.
export const annotationCardBase =
  'm-2 flex min-w-0 max-w-[min(680px,calc(100%_-_1rem))] gap-2.5 rounded-xl border border-[var(--peekdiff-annotation-border,var(--color-border))] bg-[var(--peekdiff-annotation-bg,var(--color-card))] bg-clip-padding p-3 font-sans text-[var(--peekdiff-annotation-fg,var(--color-card-foreground))] shadow-[var(--peekdiff-annotation-shadow,0_2px_4px_rgb(0_0_0_/_0.025),0_4px_8px_rgb(0_0_0_/_0.025))]';
